...
// 					System.out.println("email "+email+" name "+name1);
					
					if (email != null) {
				%>
				<li class="dropdown"><a href="#" class="dropdown-toggle"
							data-toggle="dropdown" role="button" aria-haspopup="true"> <%="Hi  " +email %><span class="caret"></span></a>
							<ul class="dropdown-menu">
								<li><a href="./IndividualOwnerHome.jsp">My Account</a></li>
		...
		ArrayList<ServiceEndUser> verifiedDetails=(ArrayList<ServiceEndUser>)request.getAttribute("verifiedDetails");
	 	ServiceEndUser serviceEndUser= verifiedDetails.get(0);
		%>
		<br>
		<br>
		<br>
...
<%-- 		<div class="container">
	...
	String serviceCenter=(String)request.getAttribute("serviceCenter");
  ...
  <table class="table">
    ...
    <tbody>
    <tr>
     <th>Vehicle Type</th>  <td><%=(serviceEndUser.getVEHICLE_TYPE().equals("2,"))?"TWO WHEELER":(serviceEndUser.getVEHICLE_TYPE().equals("4,"))?"FOUR WHEELER":(serviceEndUser.getVEHICLE_TYPE().equals("4,2,"))?"TWO WHEELER & FOUR WHEELER":"N/A"%></td>
        <th>Vehicle Brand</th><td><%=serviceEndUser.getVEHICLE_BRAND()==null?"N/A":serviceEndUser.getVEHICLE_BRAND() %></td>
         <th>Vehicle Modal</th><td><%=serviceEndUser.getMODEL()==null?"N/A":serviceEndUser.getMODEL() %></td>
        <th>Name</th> <td><%=serviceEndUser.getNAME() %></td>

    </tr>
      <tr>
       <th>Mobile No</th>  <td><%=serviceEndUser.getMOBILE_NO() %></td>
        <th>Vehicle Number</th><td><%=serviceEndUser.getCAR_NO() %></td>
         <th>Appointment Date</th><td><%=serviceEndUser.getAPPOINTMENT_DATE() %></td>
        <th>Service Center</th> <td><%=serviceEndUser.getSERVICE_CENTER() %></td>
      </tr>
       <%} %> 
    </tbody>
			</table>
  ...
  <p><font color="red"><%=message %></font></p>  
        ...
        <th>Service Center</th> <td><%=serviceEndUser.getSERVICE_CENTER() %></td>
			 ...
			 <%}else{ %>
			<p><font color="red" size="4"> allready verified..</font></p>
			 <%} %>
		</div>
