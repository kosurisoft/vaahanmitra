...
<!-- 						<div class="card-block" style="border-bottom: 3px solid #a3b745;"> -->
							<section class="example">
								...
								<form action="./DriverReports" method="post">
										...
										<%
												...
												Set<String> tripSet=driverDtls.getDriverTripId(userId);
										...
										%>
											...
											<td><b>Trip Id</b></td>
											<td><b>Traveling Hours</b></td>
											<td><b>Waiting Hours</b></td>
											<td><b>Submit</b></td>
											<td><b>Show Bill</b></td>
											<!-- <td><b>Confirm</b></td>
...
<div class="container-fluid">
  ...
  <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal<%=i %>" style="background-color: #a3b745;width:100%;">${endUserDetails.DRIVER_TRIP_ID}</button>
  ...
  <div class="modal fade" id="myModal<%=i %>" role="dialog">
    <div class="modal-dialog "> <!-- modal-sm -->
      ...
      <tr>
      <td>Name</td>
      <td>${endUserDetails.NAME}</td>
      </tr>
      <tr>
      <td>Email</td>
      <td>${endUserDetails.EMAIL}</td>
      </tr>
      <tr>
      <td>Mobile Number</td>
      <td>${endUserDetails.MOBILE_NO}</td>
      </tr>
       <tr>
      ...
      <td>${endUserDetails.APPOINTMENT_TIME}</td>
      </tr>
      <tr>
      <td>Appointment Id</td>
      <td>${endUserDetails.APPOINTMENT_ID}</td>
      </tr>
      </table>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
											...
											<form action="./InsertDriverFeedBack" method="post">
											
											<input type="hidden" name="apptId" value="${endUserDetails.APPOINTMENT_ID}"/>
											<input type="hidden" name="dtripId" value="${endUserDetails.DRIVER_TRIP_ID}"/>
											<input type="hidden" name="date" value="<%=date%>"/>
											<input type="hidden" name="tripId" value="<%=tripId%>"/>
											<input type="hidden" name="page" value="<%=page1%>"/>
											
											<td><input type="text" name="noOfHour" placeholder="Number Of Hours" required/></td>
											<td><input type="text" name="waitingHour" placeholder="Waiting Hours" required/></td>
											
											<%
											String billId=driverDtls.getDriverBillId(apptId, dtripId, userId);
											if(billId!=null){
											%>
											
											<td><input type="submit" name="submit" value="Avl Bill" class="btn btn-info" disabled="disabled" width="100%"/></td>
											
											<%}else{ %>
											
											<td><input type="submit" name="submit" value="Submit" class="btn btn-info" width="100%"/></td>
											...
											<form action="./ShowDriverBill" method="post" target="_blank">
											...
											<td><input type="submit" name="submit" value="No Bill" class="btn btn-info btn-lg" style="background-color: #a3b745;padding-top: 5px;padding-bottom: 5px;width:100%;" disabled="disabled"/></td>
...
<!-- 											</td> -->
						...
						<%-- 				<td style="float: left" ><a href="./SearchHDriver?page=${k}&state=<%=state%>&dist=<%=dist%>&permitType=<%=permitType%>&range=<%=range%>&licenseType=<%=licenseType%>">${k}&nbsp;&nbsp; </a></td> --%>
												...
												<c:set var="currentPage" value="${currentPage}"></c:set>
						             			...
						             			%>
												<a href="./DriverReports?page=${k}&date=<%=date%>&tripId=<%=tripId %>" style="color:#fc6116;"><u>${k}&nbsp;</u></a>             			
												<%} else{%>
												             			
												<a href="./DriverReports?page=${k}&date=<%=date%>&tripId=<%=tripId %>">${k}&nbsp;</a> 				
												<%} %>
