...
<!-- 						<div class="card-block" style="border-bottom: 3px solid #a3b745;"> -->
							<section class="example">
								...
								<form action="./SearchCarService" method="post">
									<div class="col-md-12" style="background: #523f6d;color: #f5f5f5;text-align: center;font-weight: bolder;height:44px;margin-top:8px;padding: 10px;">
										...
										String carNo1=(String)request.getAttribute("carNo");
										String user=(String)session.getAttribute("user");
										GetServiceCenterDetails service=new  GetServiceCenterDetails();
										Set<String> apptId=service.getCarApptId(user);
										Set<String> carNo=service.getCarNo(user);
										%>
										...
										<br>
												...
												<select name="carNo" class="form-control boxed">
												
													<option value="">Car Number</option>
													<option value="All">All</option>
												<%
												 Iterator carNoList=carNo.iterator();
												while(carNoList.hasNext()){
													String tCarNo=(String)carNoList.next();
												
												%>
												<option><%=tCarNo %></option>
												<%} %>
...
<!-- 											<input type="submit" value="Give Your Feedback" class="btn btn-primary" style="background-color: #a3b745; border: 1px solid #a3b745;"/> -->
											<% if(billId!=null && billId.size()==0){%>
											<a onclick="window.open('./carServiceBill.jsp?apptId=${endUserDetails.APPOINTMENT_ID}&vehicleType=4,','mywindow','width=2000,height=700')" class="btn btn-primary" style="background-color: #a3b745; border: 1px solid #a3b745;">Generate Your Bill</a>
											<%}else{ %>
						...
						<%-- 				<td style="float: left" ><a href="./SearchHDriver?page=${k}&state=<%=state%>&dist=<%=dist%>&permitType=<%=permitType%>&range=<%=range%>&licenseType=<%=licenseType%>">${k}&nbsp;&nbsp; </a></td> --%>
												...
												<c:set var="currentPage" value="${currentPage}"></c:set>
						             			...
						             			%>
												<a href="./SearchCarService?page=${k}&apptId=<%=apptId1%>&carNo=<%=carNo1 %>" style="color:#fc6116;"><u>${k}&nbsp;</u></a>             			
												<%} else{%>
												             			
												<a href="./SearchCarService?page=${k}&apptId=<%=apptId1%>&carNo=<%=carNo1 %>">${k}&nbsp;</a> 				
												<%} %>
