							...
							<div class="table-responsive well" style="border-radius:4%">
								<form action="./UpdateSpareParts" method="POST"
									...
									<select id="year1" class="form-control" name="year1">
													<option><%=sp.getMODEL_YEAR()%></option>
												</select>
									</div>
									<div class="col-md-2">
...
document.getElementById("year1").innerHTML = options;
	...
	<script language="javascript">
 	...
 	<script>
		var bikeBrand="<%=sp.getVEHICLE_BRAND()%>";
		$("select[name=bikeBrand] option:first").val(bikeBrand);
		$("select[name=bikeBrand] option:first").text(bikeBrand);
	
		var bikeModel="<%=sp.getVEHICLE_MODEL()%>";
		$("select[name=bikeModel] option:first").val(bikeModel);
		$("select[name=bikeModel] option:first").text(bikeModel);
