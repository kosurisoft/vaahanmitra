...
%>
<%@ page language="java" import="com.vaahanmitra.model.*,java.util.*" contentType="text/html; charset=ISO-8859-1"
							...
							<div class="table-responsive well" style="border-radius:4%">
							  ...
							  <div class="col-md-2" style="padding:0px 0px;"></div>
								...
								<form action="./UploadBulkSpareparts" method="post" enctype="multipart/form-data">
								    <input style="width: 17px; height: 17px;" id='vehicleType' value="2," name='vehicleType' type='radio' checked /> <span style="font-size: 16px; font-weight: 500;"> Two Wheeler</span> &nbsp; 
								    <input style="width: 17px; height: 17px;" id='vehicleType' value="4," name='vehicleType' type='radio' /> <span style="font-size: 16px; font-weight: 500;"> Four Wheeler</span>
                               <div id='show-me'>
                                    ...
                                    <div class="form-group col-md-5 col-sm-6 two">
						            <label for="name" class="one">Upload File :</label>
		        				...
		        				<th class="numeric">PARTNO</th>
		        				<th class="numeric">SPECIAL NAME</th>
		        				<th class="numeric">MANUFACTURER</th>
		        				<th class="numeric">SAVED/UPDATED</th>
		        			    </tr>
		        		        </thead>
		        		        
        		       <tbody>
        		       <% 
        						ArrayList<SpareParts> al = (ArrayList<SpareParts>) request.getAttribute("spareParts");
        						for(int i=0;i<al.size();i++)
        						{
        							SpareParts sp=al.get(i);
        						%>
        			   <tr>

        				<td data-title="Request ID" class="numeric"><a href="#" style="font-size: 17px;
                         color: #ff4444;  text-decoration: underline; padding: 9px;"><%=sp.getSKU() %></a></td>
        				<td data-title="Request Date" class="numeric"><%=sp.getVEHICLE_BRAND() %></td>
        				<td data-title="Retailer ID" class="numeric"><%=sp.getVEHICLE_MODEL() %></td>
        				<td data-title="Phone Number" class="numeric"><%=sp.getPARTNO() %></td>
        				<td data-title="Mail Id" class="numeric"><%=sp.getSP_NAME() %></td>
        				<td data-title="Request status" class="numeric"><%=sp.getMANUFACTURE_COMPANY_NAME() %></td>
        				<td data-title="Request status" class="numeric">
        				<%
        							if(sp.getSTATUS().equals("E")) 
     									{
     										out.println("UPDATED");
     									}
        							else
